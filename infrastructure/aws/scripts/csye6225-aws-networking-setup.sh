...
SubnetId1=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.0.0/24 --availability-zone us-east-1a|grep SubnetId|cut -d'"' -f4)
echo $SubnetId1
SubnetId2=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.1.0/24 --availability-zone us-east-1b|grep SubnetId|cut -d'"' -f4)
echo $SubnetId2
SubnetId3=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.2.0/26 --availability-zone us-east-1c|grep SubnetId|cut -d'"' -f4)
echo $SubnetId3
SubnetId4=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.3.0/24 --availability-zone us-east-1d|grep SubnetId|cut -d'"' -f4)
echo $SubnetId4
SubnetId5=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.4.0/24 --availability-zone us-east-1e|grep SubnetId|cut -d'"' -f4)
echo $SubnetId5
SubnetId6=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.0.5.0/26 --availability-zone us-east-1f|grep SubnetId|cut -d'"' -f4)
echo $SubnetId6
...
echo $AssociationId3
AssociationId4=$(aws ec2 associate-route-table --route-table-id $RouteTableId --subnet-id $SubnetId4|grep AssociationId|cut -d'"' -f4)
echo $AssociationId4
AssociationId5=$(aws ec2 associate-route-table --route-table-id $RouteTableId --subnet-id $SubnetId5|grep AssociationId|cut -d'"' -f4)
echo $AssociationId5
AssociationId6=$(aws ec2 associate-route-table --route-table-id $RouteTableId --subnet-id $SubnetId6|grep AssociationId|cut -d'"' -f4)
echo $AssociationId6
